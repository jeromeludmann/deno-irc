...
import { type AnyPlugins, createPlugin, loadPlugins } from "./plugins.ts";
...
describe("core/plugins", (test) => {
  ...
  test("load plugins by resolving dependencies", () => {
    ...
    const fakePlugin = (name: string, deps: AnyPlugins = []) =>
    ...
    const plugins: AnyPlugins = [p3, p1, p2];
  ...
  test("do not load the same plugin twice", () => {
    ...
    const plugins: AnyPlugins = [p1, p1, p2];
    loadPlugins(null as unknown as CoreClient, {}, plugins);
